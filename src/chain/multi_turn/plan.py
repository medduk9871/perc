...
import src.chain.multi_turn.templates.humaneval_java as HUMANEVAL_LUA_TEMPLATES
...
class DraftPlanChain(Chain):
    ...
    def _call(
        ...
        example_messages = prefix_messages + [self.templates.DRAFT_PLAN.ai]
        ...
        if llm.n == 1:
            ...
            code = {"draft_plan": [g[0].text for g in generations]}
        else:
            generations = chain.generate([inputs]).generations[0]
            code = {"draft_plan": [g.text for g in generations]}

        return code


class FinalPlanChain(Chain):
    dataset_type: str
    llm_kwargs: Dict[str, Any]
    examples: Optional[Dict[str, List[Dict[str, str]]]] = None
    verbose: bool = False
    ...
    def _call(
        ...
        example_messages = prefix_messages + [self.templates.FINAL_PLAN.ai]
        ...
        if "all" in self.examples:
            ...
            fewshot_examples = self.examples[inputs['id']][:1]
        fewshot_prompt = FewShotChatMessagePromptTemplate(
            example_prompt=example_prompt,
            examples=fewshot_examples
        )
        ...
        if llm.n == 1:
            ...
            code = {"final_plan": [g[0].text for g in generations]}
        else:
            generations = chain.generate([inputs]).generations[0]
            code = {"final_plan": [g.text for g in generations]}
